...
public class CheckSquare
{
        ...
        for (int y = 0; y < boardHeight; y++)
        {
            for (int x = 0; x < boardWidth; x++)
            {
                if (currentX == x && currentY == y){}
                    ...
                    board.getSquareAt(x, y).setImage(CheckSquare.class.getResource("/images/bomb.png"));
                else if(((SmartSquare) board.getSquareAt(x, y)).getGuessThisSquareIsBomb())
                    board.getSquareAt(x, y).setImage(CheckSquare.class.getResource("/images/flagWrong.png")); // Wrong guess!
            }
        }
    }
    ...
    protected void countBomb(int currentX, int currentY)
    {
         ...
         */
        if (count != 0)
            currentObject.setImage(CheckSquare.class.getResource( "/images/" + count + ".png"));
        else {
            // Paint current square as blank.
            currentObject.setImage(CheckSquare.class.getResource("/images/0.png"));
